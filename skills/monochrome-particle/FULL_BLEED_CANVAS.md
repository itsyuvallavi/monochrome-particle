...
Use **`THREE.OrthographicCamera`**. Set `left` / `right` / `top` / `bottom` from the drawable width/height and the same **`GRID_BUFFER`** used when placing particles (half-extents `width/2 + GRID_BUFFER`, `height/2 + GRID_BUFFER`). Particle positions must be built from the same width/height and buffer.

Apply **`zoom`** by narrowing those half-extents (divide by `zoom`; values **`> 1`** zoom in) **without** regenerating particle positions—only update the projection matrix / frustum, matching the shipped example.
...
3. Updates the orthographic frustum (including **`zoom`**)
...
- **`points.frustumCulled = false`** avoids points disappearing at the frustum edge with large point sizes.
